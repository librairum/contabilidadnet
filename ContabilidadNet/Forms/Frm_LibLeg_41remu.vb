...
Option Explicit On
Option Strict On
Public Class Frm_LibLeg_41remu
...
#Region "Traer Informacion de Base de Datos"
    ...
    Sub Traeimportacion()
        Try
            vistahelp = objSql.TraerDataTable("Spu_Con_Trae_PerAntlc41Remuneraciones", gbcodempresa).DefaultView
    ...
    Sub TraeEstructura()
        Try
            Vista = objSql.TraerDataTable("Spu_Con_Trae_lc41Remuneraciones", gbcodempresa, gbano, gbmes).DefaultView
        ...
        Try
            ...
            Select Case index
                Case 0
                    ...
                    Me.TraeCta("41")
                Case 1
                    If (rbtorigen_0.Checked = False And rbtorigen_1.Checked = False) Then MessageBox.Show("Origen No Valido") : Exit Sub
                    ' Configuro las Columnas de la grilla
                    If rbtorigen_0.Checked = True Then
                        tblhelp.Columns(0).DataField = "CodigoEmpleado"
                        tblhelp.Columns(1).DataField = "nombre"
                        tblhelp.Columns(2).DataField = "Tipo_de_documento"
                        tblhelp.Columns(3).DataField = "Numero_de_documento"
                        Me.traeTrabajadores()
                    Else
                        ' Configuro las Columnas de la grilla
                    ...
                    End If
                Case 2 'Importar
                    tblhelp.Columns(0).DataField = "lc41Anio"
                    tblhelp.Columns(1).DataField = "lc41Mes"
                    Me.Traeimportacion()
            End Select
    ...
    Sub AsignarAyuda(ByVal indice As Integer, ByVal tblhelp_p As C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        Try
            Select Case indice
                ...
                Case 1
                    If rbtorigen_0.Checked = True Then
                        txttrabajador.Text = tblhelp.Columns("CodigoEmpleado").Value.ToString
                        lblhelp_1.Text = tblhelp.Columns("nombre").Value.ToString
                        txttipdoc.Text = tblhelp.Columns("Tipo_de_documento").Value.ToString
                        txtnumdoc.Text = tblhelp.Columns("Numero_de_documento").Value.ToString
                        txttrabajador.Focus()
                    Else
                ...
                Case 2
                    ...
                    anioanterior = tblhelp.Columns("lc41Anio").Value.ToString
                    mesanterior = tblhelp.Columns("lc41Mes").Value.ToString
                    ...
                    If MessageBox.Show("¿Esta Seguro de importar datos del año : " & anioanterior, "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        ...
                        a = objSql.Ejecutar2("Spu_Con_Ins_PerAntlc41Remuneraciones", gbcodempresa, gbano, gbmes, anioanterior, mesanterior, "")
    ...
    Sub cargarDatos(ByVal filaactiva As DataRowView)
        Try
            ...
            If Not filaactiva Is Nothing Then
                txtcuenta.Text = filaactiva("lc41CtaCble").ToString
                lblhelp_1.Text = filaactiva("ccm01des").ToString
                txtnombrecuenta.Text = filaactiva("lc41CtaCbleDes").ToString
                '
                If filaactiva("lc41OriTra").ToString = "P" Then
                    rbtorigen_0.Checked = True
                    rbtorigen_1.Checked = False
                Else
                    rbtorigen_0.Checked = False
                    rbtorigen_1.Checked = True
                End If
                correlativo = CInt(filaactiva("lc41Correlativo").ToString)
                txttrabajador.Text = filaactiva("lc41Codigo").ToString
                lblhelp_1.Text = filaactiva("lc41Nombres").ToString
                txttipdoc.Text = filaactiva("lc41DocIdeTipo").ToString
                txtnumdoc.Text = filaactiva("lc41DocIdeNumero").ToString
                txtimporte.Text = filaactiva("lc41Importe").ToString
...
#End Region
    ...
    Private Sub Frm_LibLeg_41remu_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Me.Text = Frm_LibrosLegales.rbtrepauxiliares_11.Text
    ...
    Sub eliminar()
        ...
        Try
            ...
            identificadorAnt = FilaDeTablaAnterior("lc41Correlativo").ToString
            identificador = FilaDeTabla("lc41Correlativo").ToString
            '
            Beep()
            ...
            Dim a As Array = Array.CreateInstance(GetType(Object), 2, 10)
            a = objSql.Ejecutar2("Spu_Con_Del_lc41Remuneraciones", gbcodempresa, gbano, gbmes, correlativo, "")

            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("lc41Correlativo", identificadorAnt, tblconsulta, Vista)
            Else
                'No hago nada
            End If
        Catch ex As Exception
    ...
    Private Sub btngrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngrabar.Click
        Try
            ...
            flagorigen = If(rbtorigen_0.Checked = True, "P", "C")
            '=========================================================
            If accionMante = "N" Then
                a = objSql.Ejecutar2("Spu_Con_Ins_lc41Remuneraciones", gbcodempresa, gbano, gbmes, correlativo, txtcuenta.Text, txtnombrecuenta.Text, flagorigen, txttipdoc.Text, txtnumdoc.Text, txttrabajador.Text, lblhelp_1.Text, CDbl(txtimporte.Text), "")
            ElseIf accionMante = "M" Then
                a = objSql.Ejecutar2("Spu_Con_Upd_lc41Remuneraciones", gbcodempresa, gbano, gbmes, correlativo, txtcuenta.Text, txtnombrecuenta.Text, flagorigen, txttipdoc.Text, txtnumdoc.Text, txttrabajador.Text, lblhelp_1.Text, CDbl(txtimporte.Text), "")
                ' Asigno los Valores a los Parametros del Query
            Else
            ...
            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("lc41Correlativo", CStr(correlativo), tblconsulta, Vista)
                Me.HabilitarMantenimiento(False)
            Else
                'No hago nada
            End If
