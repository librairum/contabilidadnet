Option Explicit On
Option Strict On
Public Class Frm_LibLeg_34Intangibles
...
#Region "Traer Informacion de Base de Datos"
    ...
    Sub Traeimportacion()
        Try
            vistahelp = objSql.TraerDataTable("Spu_Con_Trae_PerAntlc34Intangibles", gbcodempresa).DefaultView
            tblhelp.SetDataBinding(vistahelp, Nothing, True)
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    ...
    Sub TraeEstructura()
        Try
            Vista = objSql.TraerDataTable("Spu_Con_Trae_lc34Intangibles", gbcodempresa, gbano, gbmes).DefaultView
...
#Region "Metodos de Mantenimiento"
            ...
            Select Case indice
                Case 0
                    If txttipo.Text = "" Then
                        lblhelp_0.Text = ""
                    Else
                        lblhelp_0.Text = Mod_BaseDatos.DameDescripcion("07" & txttipo.Text, "GLO02")
                    End If
                Case 1
                    If txtctactblevalor.Text = "" Then
                        lblhelp_1.Text = ""
                    Else
                        lblhelp_1.Text = Mod_BaseDatos.DameDescripcion(gbano$ + Trim(txtctactblevalor.Text), "CT")
                    End If
                Case 2
                    If txtctactbleamortizacion.Text = "" Then
                        lblhelp_2.Text = ""
                    Else
                        ' lblhelp_2.Text = Mod_BaseDatos.DameDescripcion("37" + txtDescripcion.Text, "GL")
                        lblhelp_2.Text = Mod_BaseDatos.DameDescripcion(gbano$ + Trim(txtctactbleamortizacion.Text), "CT")
                    End If
    ...
    Sub TraerAyuda(ByVal index As Integer, ByVal boton As Button)
        ...
        Try
            ...
            tblhelp.Columns(0).Text = "Código"
            tblhelp.Columns(1).Text = "Descripción"

            Select Case index
                Case 0
                    tblhelp.Columns(0).DataField = "glo02Codigo"
                    tblhelp.Columns(1).DataField = "glo02descripcion"
                    Me.TraeTablasGloables2("07")
                Case 1
                ...
                Case 2
                    ...
                    Me.TraeCta("39")
                Case 3 'Importar
                    tblhelp.Columns(0).DataField = "lc34Anio"
                    tblhelp.Columns(1).DataField = "lc34Mes"
                    Me.Traeimportacion()
            End Select
    ...
    Sub AsignarAyuda(ByVal indice As Integer, ByVal tblhelp_p As C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        Try
            Select Case indice
                Case 0
                    txttipo.Text = tblhelp.Columns("glo02Codigo").Value.ToString
                    lblhelp_0.Text = tblhelp.Columns("glo02descripcion").Value.ToString
                    txttipo.Focus()

                Case 1
                    txtctactblevalor.Text = tblhelp.Columns("ccm01cta").Value.ToString
                    lblhelp_1.Text = tblhelp.Columns("ccm01des").Value.ToString
                    txtctactblevalor.Focus()
                Case 2
                    txtctactbleamortizacion.Text = tblhelp.Columns("ccm01cta").Value.ToString
                    lblhelp_2.Text = tblhelp.Columns("ccm01des").Value.ToString
                    txtctactbleamortizacion.Focus()
                Case 3
                    Dim anioanterior As String
                    Dim mesanterior As String
                    anioanterior = tblhelp.Columns("lc34Anio").Value.ToString
                    mesanterior = tblhelp.Columns("lc34Mes").Value.ToString
                    ...
                    If MessageBox.Show("¿Esta Seguro de importar datos del año : " & anioanterior, "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        ...
                        a = objSql.Ejecutar2("Spu_Con_Ins_PerAntlc34Intangibles", gbcodempresa, gbano, gbmes, anioanterior, mesanterior, "")
    ...
    Sub cargarDatos(ByVal filaactiva As DataRowView)
        Try
            ...
            If Not filaactiva Is Nothing Then
                ...
                correlativo = CInt(filaactiva("lc34Correlativo").ToString)
                mskFecDoc.Text = filaactiva("lc34fecIni").ToString
                txtDescripcion.Text = filaactiva("lc34Descripcion").ToString
                txttipo.Text = filaactiva("lc34tipIntangible").ToString
                Me.TraeDameDescripcion(0)
                txtctactblevalor.Text = filaactiva("lc34CtaCble").ToString
                Me.TraeDameDescripcion(1)
                txtctactbleamortizacion.Text = filaactiva("lc34AmorCtaCble").ToString
...
#End Region
    ...
    Private Sub Frm_LibLeg_Balgen_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Try
            Mod_Mantenimiento.Formatodegrilla(tblconsulta)
            Me.Text = Frm_LibrosLegales.rbtrepauxiliares_9.Text
    ...
    Sub eliminar()
        ...
        Try
            ...
            identificadorAnt = FilaDeTablaAnterior("lc34Correlativo").ToString
            identificador = FilaDeTabla("lc34Correlativo").ToString
            '
            Beep()
            ...
            Dim a As Array = Array.CreateInstance(GetType(Object), 2, 10)
            a = objSql.Ejecutar2("Spu_Con_Del_lc34Intangibles", gbcodempresa, gbano, gbmes, correlativo, "")

            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("lc34Correlativo", identificadorAnt, tblconsulta, Vista)
            Else
                'No hago nada
            End If
        Catch ex As Exception
    ...
    Private Sub btngrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngrabar.Click
        ...
        Try
            If Not IsDate(mskFecDoc.Text) Then MsgBox("ERROR ::  Fecha No Valida", vbCritical, "Error de Usuario") : Exit Sub
            '
            If (lblhelp_0.Text = "" Or lblhelp_0.Text = "???") Then MessageBox.Show("ERROR :: Cuenta No Valida ", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
            If (lblhelp_1.Text = "" Or lblhelp_1.Text = "???") Then MessageBox.Show("ERROR :: Cuenta No Valido ", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
            If (lblhelp_2.Text = "" Or lblhelp_2.Text = "???") Then MessageBox.Show("ERROR :: Cuenta No Valido ", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
            If (txtDescripcion.Text = "" Or txtDescripcion.Text = "???") Then MessageBox.Show("ERROR :: Documento de Identidad No Valido ", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
            ...
            If accionMante = "N" Then
                a = objSql.Ejecutar2("Spu_Con_Ins_lc34Intangibles", gbcodempresa, gbano, gbmes, correlativo, mskFecDoc.Text, txtDescripcion.Text, txttipo.Text, txtctactblevalor.Text, txtctactbleamortizacion.Text, "")
            ElseIf accionMante = "M" Then
                a = objSql.Ejecutar2("Spu_Con_Upd_lc34Intangibles", gbcodempresa, gbano, gbmes, correlativo, mskFecDoc.Text, txtDescripcion.Text, txttipo.Text, txtctactblevalor.Text, txtctactbleamortizacion.Text, "")
                ' Asigno los Valores a los Parametros del Query
            Else
            ...
            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("lc34Correlativo", CStr(correlativo), tblconsulta, Vista)
                Me.HabilitarMantenimiento(False)
            Else
                'No hago nada
            End If
