...
Option Explicit On
Option Strict On
Public Class Frm_LibLeg_3749ctas
...
#Region "Traer Informacion de Base de Datos"
    ...
    Sub Traeimportacion()
        Try
            vistahelp = objSql.TraerDataTable("Spu_Con_Trae_PerCuenta3749", gbcodempresa).DefaultView
    ...
    Sub TraeEstructura()
        Try
            Vista = objSql.TraerDataTable("Spu_Con_Trae_lc37y49activoypasivodiferido", gbcodempresa, gbano, gbmes).DefaultView
        ...
        End Try
    End Sub
    Sub TraeCta(ByVal cta As String, ByVal cta2 As String)
        Try
            vistahelp = objSql.TraerDataTable("Spu_Con_Help_ctasxclase", gbcodempresa, gbano, cta, cta2, "*", "*").DefaultView
            tblhelp.SetDataBinding(vistahelp, Nothing, True)
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
        ...
        Try
            Select Case indice
                Case 0
                    ...
                    txtcuenta.Focus()
                Case 1
                    txtDescripcion.Text = tblhelp.Columns("ccm02cod").Value.ToString
    ...
    Sub cargarDatos(ByVal filaactiva As DataRowView)
        Try
            ...
            If Not filaactiva Is Nothing Then
                correlativo = CInt(filaactiva("lc3749Correlativo").ToString)
                txtcuenta.Text = filaactiva("lc3749CtaContableCod").ToString
                txtnombrecuenta.Text = filaactiva("lc3749CtaContableDesc").ToString
                txtDescripcion.Text = filaactiva("lc3749DescOperacion").ToString
                txtSaldoFinal.Text = filaactiva("lc3749SaldoFinal").ToString
                txtAdicion.Text = filaactiva("lc3749Adiciones").ToString
                txtDeduccion.Text = filaactiva("lc3749Deducciones").ToString
...
#End Region
    ...
    Private Sub Frm_LibLeg_41remu_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Me.Text = Frm_LibrosLegales.rbtrepauxiliares_15.Text
    ...
    Sub eliminar()
        ...
        Try
            ...
            identificadorAnt = FilaDeTablaAnterior("lc3749Correlativo").ToString
            identificador = FilaDeTabla("lc3749Correlativo").ToString
            '
            Beep()
            ...
            Dim a As Array = Array.CreateInstance(GetType(Object), 2, 10)
            a = objSql.Ejecutar2("Spu_Con_Del_lc37y49activoypasivodiferido", gbcodempresa, gbano, gbmes, correlativo, "")

            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("lc3749Correlativo", identificadorAnt, tblconsulta, Vista)
            Else
                'No hago nada
            End If
        Catch ex As Exception
    ...
    Private Sub btngrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngrabar.Click
        Try
            ...
            If accionMante = "N" Then
                a = objSql.Ejecutar2("Spu_Con_Ins_lc37y49activoypasivodiferido", gbcodempresa, gbano, gbmes, txtcuenta.Text, txtnombrecuenta.Text, txtDescripcion.Text, CDbl(txtSaldoFinal.Text), CDbl(txtAdicion.Text), CDbl(txtDeduccion.Text), "")
            ElseIf accionMante = "M" Then
                a = objSql.Ejecutar2("Spu_Con_Upd_lc37y49activoypasivodiferido", gbcodempresa, gbano, gbmes, correlativo, txtcuenta.Text, txtnombrecuenta.Text, txtDescripcion.Text, CDbl(txtSaldoFinal.Text), CDbl(txtAdicion.Text), CDbl(txtDeduccion.Text), "")
                ' Asigno los Valores a los Parametros del Query
            Else
