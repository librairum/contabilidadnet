...
#Region "Metodos de Mantenimiento"
    ...
    Sub HabilitarMantenimiento(ByVal valor As Boolean)
        Try
            ...
            txtNombre.ReadOnly = If(Me.accionMante = "M", True, Not valor)
            txtClave.ReadOnly = If(Me.accionMante = "M", True, Not valor)
    ...
    Sub cargarDatos(ByVal filaactiva As DataRowView)
        Try
            ...
            If Not filaactiva Is Nothing Then
                ...
                txtNombreComp.Text = filaactiva("NombreComp").ToString
                txtCargo.Text = filaactiva("Cargo").ToString

                txtClave.Text = filaactiva("Clave").ToString
                txtClave.Text = Funciones.Funciones.Desencripta(txtClave.Text.Trim)

                If filaactiva("AccPerCerr").ToString = "S" Then
                    chkAccPerCerr.Checked = True
                Else
                    chkAccPerCerr.Checked = False
                End If
                'Valores que no se suan en la contabilidad pero se pueden usar en otrso sistemas
                AccPerCon = filaactiva("AccPerCon").ToString
                VarImpChe = filaactiva("VarImpChe").ToString
...
#End Region
    ...
    Sub eliminar()
        ...
        Try
            ...
            CodigoActivoAnt = FilaDeTablaAnterior("Nombre").ToString
            CodigoActivo = FilaDeTabla("Nombre").ToString
            '
            Beep()
            If MessageBox.Show("AVISO :: Esta seguro de Eliminar el REGISTRO : " & txtNombre.Text.Trim, "Eliminar Registro", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then Exit Sub
            Me.accionMante = "E"
            ' Asigno los Valores a los Parametros del Query
            Dim a As Array = Array.CreateInstance(GetType(Object), 2, 10)
            a = objSql.Ejecutar2("sp_Glo_Del_Usuario", gbc_sistema, txtNombre.Text.Trim, "")

            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("Nombre", CodigoActivoAnt, tblconsulta, Vista)
            Else
                'No hago nada
            End If
        Catch ex As Exception
    ...
    Private Sub btngrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btngrabar.Click
        Dim flagAccPerCerrado As String = ""
        Dim clave As String = ""
        Try
            'Valido usuario y clave
            If txtNombre.Text.Trim = "" Then MessageBox.Show(gbc_MensajeValidar + "Codigo no valida", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : txtNombre.Focus() : Exit Sub
            If txtClave.Text.Trim = "" Then MessageBox.Show(gbc_MensajeValidar + "Descripcion no valida", "", MessageBoxButtons.OK, MessageBoxIcon.Error) : txtClave.Focus() : Exit Sub
            ...
            If chkAccPerCerr.Checked = True Then
                flagAccPerCerrado = "S"
            Else
                flagAccPerCerrado = "N"
            End If

            'Si es nuevo por defecto es NO, Caso contrario toma su mismo vaor
            If Me.accionMante = "N" Then
                AccPerCon = "N"
                VarImpChe = "N"
            End If
            'Encripta la clave
            clave = Funciones.Funciones.Encripta(txtClave.Text.Trim)

            If accionMante = "N" Then
                a = objSql.Ejecutar2("sp_Glo_Ins_Usuario", gbc_sistema, txtNombre.Text.Trim, txtNombreComp.Text.Trim, txtCargo.Text, clave, flagAccPerCerrado, AccPerCon, VarImpChe, "")
            ElseIf accionMante = "M" Then
                a = objSql.Ejecutar2("sp_Glo_Upd_Usuario", gbc_sistema, txtNombre.Text.Trim, txtNombreComp.Text.Trim, txtCargo.Text, clave, flagAccPerCerrado, AccPerCon, VarImpChe, "")
                ' Asigno los Valores a los Parametros del Query
            Else
            ...
            If Funciones.Funciones.MensajesdelSQl(a) = True Then
                Me.refrescarGrilla()
                Me.Posicionar("Nombre", txtNombre.Text.Trim, tblconsulta, Vista)
                Me.HabilitarMantenimiento(False)
            Else
                'No hago nada
            End If
